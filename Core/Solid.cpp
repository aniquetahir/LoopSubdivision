...
Solid::tEdge Solid::createEdge( int start, int end )
{
	...
	if( e != NULL ){
        // cout<<stringifyVertex(e->halfedge(0)->source())<<"->"<<stringifyVertex(e->halfedge(0)->target())<<endl;
	    return e;
	}
...
void Solid::edgeSwap( Edge * e )
{
    Vertex* nv1 = e->halfedge(0)->he_next()->target();
    Vertex* nv2 = e->halfedge(1)->he_next()->target();

    Vertex* ev1;
    Vertex* ev2;

    e->get_vertices(ev1, ev2);
    ...
    // Delete Half Edges and Faces around the Input Edge
    for( int j = 0; j < 2; j ++ )
    {
        tVertex v[3];
        tHalfEdge he = hes[j];
...
//        int* vid = (int*)(malloc(sizeof(int)*3));
//        tVertex w[3];
//        w[0] = nv2; w[1] = v[0]; w[2] = v[1];
...
//        }
//
//        faceVids.push_back(vid);

    }

    int* vid = (int*)(malloc(sizeof(int)*3));
    tVertex w[3];
    w[0] = nv2; w[1] = nv1; w[2] = ev1;
//    printf("\n%f %f %f\n",w[0]->point()(0),w[0]->point()(1),w[0]->point()(2));
//    printf("%f %f %f\n",w[1]->point()(0),w[1]->point()(1),w[1]->point()(2));
//    printf("%f %f %f\n",w[2]->point()(0),w[2]->point()(1),w[2]->point()(2));
    for(int k = 0; k < 3; k ++ )
    ...
    int* vid2 = (int*)(malloc(sizeof(int)*3));
    w[0] = nv2; w[1] = ev2; w[2] = nv1;
    for(int k = 0; k < 3; k ++ )
    {
        vid2[k] = w[k]->id();
    }

    faceVids.push_back(vid2);


    for(auto vid: faceVids){
...
AddedElements Solid::edgeSplitGetNewEdges( Edge * e )
{
    Vertex* v1;
    Vertex* v2;
    ...
    for( ListIterator<Face> iter( new_faces ); !iter.end(); ++ iter )
    {
        ...
        do{
            Edge     * e = he->edge();
            if(
                    !(e->halfedge(0)->source()==v1 || e->halfedge(0)->target()==v1 ||
                    e->halfedge(0)->source()==v2 || e->halfedge(0)->target()==v2))
            {
    ...
    addedEdges = removeVectorDuplicates(addedEdges);

    AddedElements elemAdded;
    elemAdded.num_edges = addedEdges.size();
    Edge** eArray = (Edge**)malloc(sizeof(Edge*)*addedEdges.size());
    for(int i=0;i<addedEdges.size();i++){
        eArray[i] = addedEdges[i];
    }
    elemAdded.edges = eArray;
    elemAdded.vertex = nv;
